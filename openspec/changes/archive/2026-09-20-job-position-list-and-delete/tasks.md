...
- [x] 1.1 Agregar `listJobPositions(employerId)` y `deleteJobPosition(jobPositionId)` a
  `JobPositionRepository` y verificar que `yarn tsc --noEmit` falle únicamente por la
  implementación REST todavía ausente
- [x] 1.2 Implementar ambos métodos en `jobPositionRepositoryRest` sobre
...
- [x] 2.1 Agregar `useEmployerJobPositions(employerId)` sobre
...
- [x] 2.2 Agregar `useDeleteJobPosition(employerId)` que invalide la colección del
...
- [x] 3.1 Agregar `src/components/ui/alert-dialog.tsx` sobre `@radix-ui/react-dialog`
...
- [x] 4.1 Crear la card de un puesto con posición, rol, experiencia requerida, nivel
...
- [x] 4.2 Crear `JobPositionsListPage` derivando el `employerId` de `useEmployerContext`,
...
- [x] 4.3 Conectar la acción de editar de cada card a `PATHS.main.employer.jobsEdit` y
  verificar que abra el formulario de edición del puesto correcto
- [x] 4.4 Conectar la acción de eliminar al `alert-dialog`, nombrando el puesto y
...
- [x] 4.5 Tratar `403` y `404` de la eliminación como desincronización invalidando la
  colección, y verificar que el puesto desaparezca en lugar de ofrecer un reintento inútil
- [x] 4.6 Verificar por inspección que ninguna pantalla del área ofrezca reapertura,
  restauración o reactivación de un puesto eliminado

## 5. Rutas y navegación

- [x] 5.1 Agregar `PATHS.main.employer.jobsCandidates(jobPositionId)` y verificar que
  `yarn tsc --noEmit` pase
- [x] 5.2 Reemplazar el `ContinuityPage` de `/main/employer/jobs` por
...
- [x] 5.3 Agregar la ruta `/main/employer/jobs/:jobPositionId/candidates` con
  ...
  sesión `employee`
- [x] 5.4 Conectar el acceso a candidatos recomendados desde cada card y verificar que
  navegue al destino del puesto correcto

## 6. Validación

- [x] 6.1 Agregar `e2e/job-position-list-and-delete.spec.ts` con rutas mockeadas
...
- [x] 6.2 Ejecutar `yarn lint`, `yarn tsc --noEmit` y `yarn build`, y verificar que los
  tres terminen sin errores
- [x] 6.3 Recorrer los criterios de aceptación de LAB-27 uno por uno y verificar cada uno
