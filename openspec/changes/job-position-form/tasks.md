...
- [ ] 1.1 Crear `src/features/job-positions/models/job-position.ts` con `JobPosition`,
  `CreateJobPosition` y los tipos de enums alineados al contrato del backend; verificar
  con `yarn tsc --noEmit`.
- [ ] 1.2 Crear `src/features/job-positions/repo/job-position-repository.ts` con
...
- [ ] 1.3 Crear `src/features/job-positions/repo/rest/types.ts` con el cuerpo y la
...
- [ ] 1.4 Crear `src/features/job-positions/repo/rest/helpers.ts` con los mappers
  respuesta → dominio y dominio → cuerpo, enviando `technical_resources` siempre como
  array; verificar con `yarn tsc --noEmit`.
- [ ] 1.5 Crear `src/features/job-positions/repo/rest/job-position-repository.rest.ts`
...
- [ ] 1.6 Exportar `jobPositionRepository` desde `src/api/index.ts` siguiendo el patrón
  de `employerRepository`; verificar que el import resuelve con `yarn tsc --noEmit`.

## 2. Estado remoto

- [ ] 2.1 Crear `src/features/job-positions/hooks/use-job-positions.ts` con
...
- [ ] 2.2 Agregar en la feature `employers` un hook de lectura del perfil propio que
...
- [ ] 3.1 Crear `src/features/job-positions/forms/job-position/options.ts` con las
...
- [ ] 3.2 Crear `src/features/job-positions/forms/job-position/schema.ts` con Zod,
...
- [ ] 3.3 Crear `job-position-form.tsx` como componente único de alta y edición,
...
- [ ] 4.4 Agregar `jobsNew` y `jobsEdit` a `src/router/paths.ts` y registrar ambas rutas
...
- [ ] 5.1 Agregar `e2e/job-position-form.spec.ts` con respuestas HTTP simuladas cubriendo
...
- [ ] 5.2 Ejecutar `yarn tsc --noEmit`, `yarn eslint` sobre los archivos tocados y
